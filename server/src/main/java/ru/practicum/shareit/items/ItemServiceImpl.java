...
import ru.practicum.shareit.bookings.BookingMapper;
...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemDto> search(String text, Long userId, Integer from, Integer size) {
        ...
        List<Item> items;
        if (from == null && size == null) {
            items = itemRepository.findByIsAvailableIsTrue(text.toLowerCase());
        } else {
            Pageable page = PageRequest.of(from, size);
            items = itemRepository.findPageByIsAvailableIsTrue(text.toLowerCase(), page).toList();
        }
    ...
    public CommentDto addComment(Long itemId, Long userId, CommentDtoCreation commentDtoCreation) {
        if (!StringUtils.hasText(commentDtoCreation.getText())) {
            throw new CommentTextIsEmptyException("Текст не может быть пустым");
        }
